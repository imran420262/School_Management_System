...
class AccountSalaryController extends Controller
{
    ...
    public function AccountSalaryGetEmployee(Request $request){
    	...
    	$date = date('Y-m',strtotime($request->date));
    	 ...
    	 $html['thsource'] .= '<th>Bonous Amount</th>';
    	 ...
    	 foreach ($data as $key => $attend) {
			...
			//  if($account_salary->bonous_amount != null){
 	...
 	$html[$key]['tdsource']  = '<td>'.($key+1).'</td>'.$key;
 	...
 	$html[$key]['tdsource'] .='<td>'.'<input type="text" name="bonous_amount[]" value="'.$bonous_amount.'" >'.'</td>';
 	$html[$key]['tdsource'] .='<td>'.'<input type="text" name="bonous_name[]" value="'.$bonous_name.'" >'.'</td>';
