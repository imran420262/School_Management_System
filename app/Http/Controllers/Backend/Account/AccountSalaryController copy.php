...
class AccountSalaryController extends Controller
{
    ...
    public function AccountSalaryGetEmployee(Request $request){
    	...
    	// $date = $request->date;
    	 ...
    	 $html['thsource'] .= '<th>Add Bonous</th>';
    	 $html['thsource'] .= '<th>Bonous Name</th>';
    	 ...
    	 foreach ($data as $key => $attend) {
			 ...
			 $bonous_amount = $account_salary->bonous_amount;
			 $bonous_name = $account_salary->bonous_name;
 	...
 	$html[$key]['tdsource'] .='<td>'.'<input type="hidden" name="emplpyee_id[]" value="'.$attend->employee_id.'">'.'<input type="text" name="bonous_amount[]" id="'.$key.'" value="'.$bonous_amount.'" '.$bonous_amount.' style="transform: scale(1.2);margin-left: 10px;"> <label for="'.$key.'"> </label> '.'</td>'; 
 	$html[$key]['tdsource'] .='<td>'.'<input type="hidden" name="employee_id[]" value="'.$attend->employee_id.'">'.'<input type="text" name="bonous_name[]" id="'.$key.'" value="'.$bonous_name.'" '.$bonous_name.' style="transform: scale(1.2);margin-left: 10px;"> <label for="'.$key.'"> </label> '.'</td>'; 
    ...
    public function AccountSalaryStore(Request $request){
    	...
    	if ($checkdata !=null) {
    		for ($i=0; $i <count($checkdata) ; $i++) { 
				...
				$data->bonous_amount = $request->bonous_amount[$checkdata[$i]];
				$data->bonous_name = $request->bonous_name[$checkdata[$i]];
    			$data->save();
    		} 
    	} // end if 
