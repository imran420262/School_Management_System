...
{
    public function StudentFeeView() { //This function is for student fee history
    	$data['allData'] = AccountStudentFee::orderBy('date','desc')->get() ; 
		$data['totalExam'] = ExamSchedule::all();
    	return view('backend.account.student_fee.student_fee_history',$data);
    }
	...
	public function StudentFeeGetStudent(Request $request) { 
		...
		foreach ($data as $key => $std) {
			...
			$html[$key]['tdsource']  .= '<td>'.$studentfee->amount.'$'.'<input type="hidden" name="date" value= " '.$date.' " >'.'</td>';
	...
	} //end of StudentFeeGetstudent method. sidebar Student Fee View 
    ...
    public function StudentFeeStore(Request $request){
    	...
    	if ($checkdata !=null) {
    		for ($i=0; $i <count($checkdata) ; $i++) { 
				...
				if($request->fee_category_id == 1) {
					$TotalFee = $data2->total_registration_fee_paid+$request->amount[$checkdata[$i]];
					$data2->total_registration_fee_paid = $TotalFee;
					$data2->save();
				} 
				if($request->fee_category_id == 2) {
					$TotalFee = $data2->total_monthly_fee_paid+$request->amount[$checkdata[$i]];
					$data2->total_monthly_fee_paid = $TotalFee;
					$data2->save();

				} 
				if($request->fee_category_id == 3) {
					$TotalFee = $data2->total_exam_fee_paid+$request->amount[$checkdata[$i]];
					$data2->total_exam_fee_paid = $TotalFee;
					$data2->save();
